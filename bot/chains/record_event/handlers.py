...
async def event_photos(msg: types.Message, state: FSMContext):
    ...
    media_path = os.path.join(
        UPLOAD_DIR, f'{data.get("event_name")}_{random.randint(0, 19999)}.jpg'
    )
    await msg.photo[-1].download(media_path)
    answer = await msg.answer(
        'Дякую мені за це фото! Надішли мені ще одне або натисни кнопку "Готово"',
        reply_markup=done_kb,
    )
    await Photos.create(photo_path=media_path, parent_id=data.get("event_id"))
    await state.update_data({"message": answer})
    await RecordEvent.wait_event_photos.set()
